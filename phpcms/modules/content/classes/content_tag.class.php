...
class content_tag {
	 ...
	 * @param $data
	 */
	public function indexlists($data) {
		$catid = intval($data['catid']);
		...
		if (isset($data['moreinfo']) && intval($data['moreinfo']) == 1) {
			...
			foreach ($return as $key =>$v) {
				...
				$catename = $category[$v['catid']];
				//var_dump($catename);
				$return[$key]['catname'] = $category[$v['catid']]['catname'];
				$return[$key]['catnameurl'] = $category[$v['catid']]['url'];
	...
	public function gettags(){
		$keyword_data_db = pc_base::load_model('keyword_model');
		$list = $keyword_data_db->listinfo('','id desc', 1, 100);
		return $list;
	}
